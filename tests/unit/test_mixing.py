...
def test_build_commentary_filter_ducks_original_and_normalizes_loudness() -> None:
    filter_graph = build_commentary_filter(original_ducking_db=-22, final_loudness_normalization=True)

    assert "volume=0.079433" in filter_graph
...
def test_build_commentary_filter_can_skip_loudness_normalization() -> None:
    filter_graph = build_commentary_filter(original_ducking_db=-22, final_loudness_normalization=False)
